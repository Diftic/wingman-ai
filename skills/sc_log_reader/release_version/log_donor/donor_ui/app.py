...
from log_donor.ui_dialog import build_preview, format_summary_header
...
class DonorServer:
    ...
    def _run_upload_job(self) -> None:
        ...
        try:
            ...
            # Record successes in the dedup store
            rows: list[tuple[str, str, str]] = []
            upload_id = upload_result.upload_id or ""
            for f in upload_result.files:
                if f.succeeded and not f.already_uploaded:
                    # Find matching candidate to get original path
                    for c in candidates:
                        if c.renamed == f.renamed or c.sha256 == f.sha256:
                            rows.append((f.sha256, str(c.path), upload_id))
                            break
            if rows:
