...
## Status: DISCONTINUED (2026-04-25)

The Regolith.Rocks site has been shut down, so this skill no longer has a working
data source for survey/deposit data. The skill is on **indefinite pause** — code is
preserved in case Regolith.Rocks (or an equivalent service) comes back online and
we choose to revive it. Until then, do not ship, install, or recommend this skill.

UEX Corp integration is still functional, but the unique value of this skill was
the Regolith survey data layered on top of UEX prices. Without Regolith, what
remains is a duplicate of `uexcorp` and not worth maintaining as a separate skill.

## Version: 1.1.0 (last working version, frozen)
