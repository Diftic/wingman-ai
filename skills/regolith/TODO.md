...
## Current Status: DISCONTINUED (2026-04-25)

Regolith.Rocks has been shut down. This skill is on indefinite pause — no further
work, no testing, no installs. If Regolith.Rocks (or an equivalent survey-data
service) returns, revisit and pick up from the "Completed" section below.

**Revival checklist (when/if we come back to this):**
- [ ] Confirm Regolith.Rocks (or replacement) is live and the API contract is unchanged
- [ ] Re-validate UEX integration (price endpoints may have moved)
- [ ] Re-test the 16 AI tools end-to-end
- [ ] Bump version, update Devlog status, and run `update_release.py`

---

## Completed (frozen at v1.1.0)
