...
Bundled dataset predates patch 4.7's mining and resource overhaul, so locations,
mineral compositions, spawn rates, and equipment stats are no longer accurate.
`auto_activate` disabled. Skill is on hold — do not act on the items below until
the data is rebuilt against 4.7. See `Devlog.md` for the revival checklist.
