...
This skill was built before Star Citizen patch 4.7's mining and resource overhaul.
The bundled mining data (locations, mineral compositions, spawn rates, equipment
stats) was extracted from the pre-4.7 build, and 4.7 reset those underlying values —
so any answers about deposits, yields, or equipment behavior are likely wrong for the
live game.

The skill itself still runs (browser dashboard, scan pipeline, etc.) — it's the
dataset that's stale. `auto_activate` has also been turned off so the outdated data
no longer auto-loads. To revive: re-extract mining data from the 4.7 build, re-run
the scan/template calibration, update Devlog status, flip `auto_activate` back on
if desired, and remove the "Outdated" tag from `default_config.yaml`.
